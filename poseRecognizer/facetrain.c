...
backprop_face(IMAGELIST* trainlist,IMAGELIST *test1list,IMAGELIST *test2list,int epochs,int savedelta,char* netname,int list_errors)
{
  IMAGE *iimg;
  BPNN *net;
...
/*** If the output unit is on, then we correctly recognized ***/
if (net->output_units[i] > 0.5) {
return (1);

/*** otherwise, we didn't recognize ***/
} else {
return (0);
}
...
/*** If the output unit is on, then we mistakenly recognized  ***/
if (net->output_units[i] > 0.5) {
return (0);

/*** else, we correctly realized that it wasn't someone ***/
} else {
return (1);
}
}
